...
function convex_hull_alg_test(hull_alg!)
    ...
    @testset "numerical problem $order" for order in [CCW, CW]
        hull = ConvexHull{order, Float64}()
        points = Point{Float64}[[-0.17559141363793285, 0.1777893970092983], [-0.17574071124767324, 0.04921003844438379], [0.08480367424885946, 0.17745906024313485], [0.08465437663911904, 0.04887970167822034], [-0.13059226250542266, 0.17773231116261107], [-0.13074156011516305, 0.04915295259769656], [-0.1753504285942606, -0.1772960226863406], [-0.1756647405301049, -0.04875022362480264], [0.08503864201675085, -0.17672638211747324], [0.08472433008090652, -0.04818058305593528], [-0.13035231731355584, -0.17719758249586814], [-0.13066662924940015, -0.04865178343433017]]
        hull_alg!(hull, points)
        @test is_ordered_and_strongly_convex(vertices(hull), order)
    end
end
